# baselines_final.py
"""
Dynamic-universe baselines for debenture portfolio OPS
------------------------------------------------------
...
• **Rewards at t** use realized returns of t (minus costs), with cash at rf_t.
...
python baselines_final.py --universe cdi --strategies EW,INDEX,RP_VOL,RP_DURATION,CARRY_TILT,MINVAR \
    --rebalance_interval 5 --max_weight 0.10 --transaction_cost_bps 10 \
...
def build_eval_panel_with_union(panel: pd.DataFrame, fold_cfg: Dict[str, str], union_ids: List[str]) -> pd.DataFrame:
    ...
    for c in SAFE_FILL_0:
        if c in test_aug.columns:
            test_aug[c] = test_aug[c].astype(float).fillna(0.0)
    if "sector_id" in test_aug.columns:
        test_aug["sector_id"] = test_aug["sector_id"].fillna(-1).astype(np.int16)
    for opt in ["index_weight","rv_residual","sector_curve_slope","sector_curve_curv","ttm_rank"]:
...
def simulate_baseline(panel: pd.DataFrame, strategy: str, cfg: BaselineConfig) -> Dict[str, pd.DataFrame]:
    ...
    for t in range(T):
        ...
        #rp_assets = float(np.dot(w_new, R[t]))
...
def main():
    ap = argparse.ArgumentParser(description="Dynamic-universe baselines for debenture OPS")
    ...
    ap.add_argument("--rebalance_interval", type=int, default=5)
    ap.add_argument("--max_weight", type=float, default=0.10)
    ap.add_argument("--transaction_cost_bps", type=float, default=10.0)
    ap.add_argument("--allow_cash", action="store_true")
    ...
    ap.add_argument("--delist_extra_bps", type=float, default=10.0)
    args = ap.parse_args()
    ...
    cfg = BaselineConfig(
        ...
        allow_cash=bool(args.allow_cash),
        on_inactive=args.on_inactive,
        roll_window=args.roll_window,
        ridge=args.ridge,
        delist_extra_bps=args.delist_extra_bps,
    )
    ...
    for fold in fold_specs:
        ...
        for strat in strat_list:
            ...
            print(f"[OK] Fold {fold_i} | {strat} written → {out_pkl}")

    print("[DONE] Baselines complete.")
    print(f"Updated:\n  {fold_metrics_csv}\n  {all_metrics_csv}\n  {all_returns_csv}\n  {all_diags_csv}")
