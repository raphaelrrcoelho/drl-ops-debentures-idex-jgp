...
def var_cvar(r: np.ndarray, level: float = 0.95) -> Tuple[float, float]:
    ...
    Returns (VaR, CVaR), both typically Ã¢â€°Â¤ 0 for left tail.
    """
    if r.size == 0:
        return (0.0, 0.0)
...
def returns_wide_by_group(df_long: pd.DataFrame) -> pd.DataFrame:
    ...
    print(f"  Created wide format: {len(wide)} dates Ãƒâ€” {len(wide.columns)} groups")
    ...
    if not pairwise_df.empty:
        ...
        for _, row in pairwise_df.iterrows():
            ...
            print(f"    {row['Strategy_A']} vs {row['Strategy_B']}: "
                  f"ÃŽâ€={row['Mean_Diff']:.3%}, DM p={row['DM_pval']:.3f} {sig}")
